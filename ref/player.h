 ...
 *  If so, remove those cards from the hand, and add the rank to the
 * book. Returns after finding one matching set of 4, so should be
 * called after adding each a new card.
 *
 *  target: pointer to the player to check
 *
 *  Return: a char that indicates the book that was added; return 0 if
 * no book added.
 */
 ...
 *
 *  rank: the rank to pl_search for
 *  target: the player (and their hand) to pl_search
 *
 *  Return: If the player has a card of that rank, return 1, else
 * return 0
 */
 ...
 * -----------------------
 *
 *   Select a rank randomly to play this turn. The player must have at
 * least one card of the selected rank in their hand.
 *
 ...
 * -------------------
 *
 *   Read standard input to get rank user wishes to play.  Must
 * perform error checking to make sure at least one card in the
 * player's hand is of the requested rank.  If not, print out "Error -
 * must have at least one card from rank to play" and then re-prompt
 * the user.
 *
